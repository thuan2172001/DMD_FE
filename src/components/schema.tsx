...
import { DatePicker, Entity, Enum, Upload } from "components";
...
function Schema({ controls, showError, value, onChange, errorFields, totalGrid = 1 }: FormViewProps, ref: any) {
  ...
  function validate(input: any) {
    ...
    controls.forEach((ctrl) => {
      if (ctrl.control === SchemaControl.Schema) {
        if (!schemas[ctrl.field]?.ref.current.validate(input[ctrl.field] || {})) {
          pass = false;
        }
      } else {
        //@ts-ignore
    ...
    <div className="block w-full bg-white">
      {showError && Object.keys(errors).length > 0 && (
        <div className="w-full rounded-md bg-primary-900 text-white mt-2 p-2">
          <p className="text-xl font-semibold">{t("Missing required fields")}: </p>
          ...
          return (
            <div className="col-span-1">
              {controls.map((ctrl, index) => {
                ...
                let isSameGrid = idx + 1 === (ctrl.grid ?? 1);

                if (!isShow || !isSameGrid) {
                  return <></>;
                }
                ...
                switch (ctrl.control) {
                  ...
                  case SchemaControl.Text:
                    return (
                      <Form.Field required={ctrl.required}>
                        <label className={`${errorFields?.includes(ctrl.field) && "error-field-schema"}`}>{t(ctrl.label)}</label>
                        <p className="text-gray-400 text-sm m-0 font-light">{t(ctrl.description)}</p>
                        <Input
                          fluid
